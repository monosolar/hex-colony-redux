...
export default class TurnCalculation {
    ...
    getPriorityCellIdx() {
        ...
        for (i = initI; limitCondition(i); i += iterator){
            ...
            currentCellFreeDirections = this.getCellFreeDirectionsArray(this._colonyOwnCells[i]);

            if (currentCellFreeDirections.length > 0)
                break;
        }

        return this.getDirectedCellIdx(cellIdx, currentCellFreeDirections[rndm(currentCellFreeDirections.length)])
    }
    ...
    getCellFreeDirectionsArray(cellIdx) {
        ...
        for (let i = 0; i < 6; i++){
            ...
            neighbourCell = d3.select(this._mapCells[this.getDirectedCellIdx(cellIdx,DIRECTIONS_ARRAY[i])]);
    ...
    isCellExisting(currentIndex,direction) {
        ...
        if (  (direction === SW || direction === SO) && (currentIndex >= (mapColumns * (mapRows-1))) )
            return false
            
        else
        if ( (  (direction === SW && !isRowOdd(currentIndex)) ||
        ...
        if ( (  (direction === SO && isRowOdd(currentIndex)) ||
